...
;             r9.1 - color
;             r9.0 - rotation
;             ra.0 - steep flag (transposed x,y values)
;
...
;  rb.1  => dy  abs(y1 - y0)
;  ra.0  => steep flag (calculate with transposed x,y)
;  r9.1  => color
;  r9.0  => rotation
;  r8    => endpoint x1, endpoint y1
;  r7    => pixel (x,y) to be drawn (either rd, or rd transposed for steep line)
;------------------------------------------------------------------------------- 
...
; r7 and r8 were exchanged by the caller so that r7 is left of r8, making the 
; difference dx = x1 - x0 always positive.
;------------------------------------------------------------------------------- 
; The logic below sets up the remaining values used by algorithm.
;------------------------------------------------------------------------------- 

            copy   r7, rd     ; copy origin to x,y
            glo    rd         ; dx = x1 - x0
            str    r2         ; x0 in M(X)
...
sl_done:    glo    ra         ; check steep flag
